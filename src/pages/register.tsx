  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Akun berhasil dibuat. Email verifikasi sedang dikirim...",
      });
      // Store email in session storage and redirect to OTP verification page
      sessionStorage.setItem("registration_email", formData.email);
      router.push(
        `/verify-otp?userId=${data.user.id}&email=${encodeURIComponent(
          formData.email
        )}`
      );
    } catch (error) {
      ...
      if (error instanceof Error) {
        if (error.message.includes("already exists")) {
          toast({
            title: "❌ Email atau Username Sudah Terdaftar",
            description:
              "Email atau username yang Anda gunakan sudah terdaftar. Silakan gunakan yang lain atau coba login.",
            variant: "destructive",
          });
